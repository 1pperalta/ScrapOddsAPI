...
const OddsComparison = ({ match }) => {
  ...
  const getOutcomeColor = (outcome) => {
    switch (outcome.toLowerCase()) {
      ...
      case match.away.toLowerCase():
        return 'bg-premier-50 border-premier-500 text-premier-500';
      case 'draw':
        return 'bg-yellow-50 border-yellow-500 text-yellow-500';
      default:
        return 'bg-gray-50 border-gray-500 text-gray-500';
    }
  };
  ...
  return (
    <div className="card">
      <div className="mb-6">
        <h3 className="text-2xl font-bold text-gray-500 mb-2 flex items-center gap-2">
          <ChartBar size={28} weight="duotone" className="text-premier-500" /> {TEXTS.oddsComparison.title}
        </h3>
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="font-medium text-gray-500">
                              {bookie.name}
                              {index === 0 && (
                                <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-500 text-white">
                          ...
                          <td className="px-4 py-3 text-right">
                            <span className="font-bold text-premier-500">
                              {bookie.odds}
                            </span>
                          </td>
      ...
      <div className="mt-6 pt-6 border-t border-gray-500">
        <h5 className="font-semibold text-gray-500 mb-3 flex items-center gap-2">
          <TrendUp size={20} weight="duotone" className="text-premier-500" /> {TEXTS.oddsComparison.summaryTitle}
        </h5>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="bg-gray-50 p-3 rounded-lg">
            <div className="text-gray-500">{TEXTS.oddsComparison.totalBookmakers}</div>
            <div className="font-bold text-lg">{match.bookmakers.length}</div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg">
            <div className="text-gray-500">{TEXTS.oddsComparison.regionsCovered}</div>
            <div className="font-bold text-lg">
              {[...new Set(match.bookmakers.map(b => b.region))].length}
            </div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg">
            <div className="text-gray-500">{TEXTS.oddsComparison.outcomesAvailable}</div>
